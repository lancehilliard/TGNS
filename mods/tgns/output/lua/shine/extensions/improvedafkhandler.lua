...
function PlayerAFK:IsImmune( Config, Player, Client )
	...
	if TGNS.PlayerIsOnPlayingTeam( Player ) and not TGNS.IsPlayerAlive( Player ) then return true end
	...
	local Players = TGNS.Where( TGNS.GetClientList(), function(c) return not TGNS.GetIsClientVirtual(c) end )
...
function PlayerAFK:PerformActionAgainst( Config, Client )
	...
	if WarnTimeSeconds then
		if (Config.KickTime - TimeAfk) > WarnTimeSeconds then
			...
			local afkSeconds = Config.KickTime - WarnTimeSeconds
			if afkSeconds > 0 then
				md:ToPlayerNotifyInfo( Player, "You will be AFK kicked in " .. WarnTimeSeconds .. " seconds." )
				LastActionTimes[Client].LastWarn = WarnTimeIndex
			end
			return true
		end
	elseif Config.KickTime - TimeAfk <= 0.01 then -- No warnings left
		TGNSClientKicker.Kick( Client, "You were kicked for being AFK for " .. string.TimeToString(Config.KickTime) .. "." )
		return true
	end
end
...
function Plugin:OnProcessMove( Player, Input )
	...
	if not ClientActionTimes then
		if not PlayerAFK:ResetAFKTimer( Client ) then return end
